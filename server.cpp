...
void getReply(string str,int client,string Str1){
    ...
    if(arr[0]=="user" || arr[0]=="USER"){
        ...
        char pt[passwordPath.size() + 1];
        strcpy(pt, passwordPath.c_str());
        ...
        name=space(getUser[0]);
        password=space(getUser[1]);
        uName=space(arr[1]);
        upass=space(arr[2]);
...
void getvalues(){
    ...
    for(int i = 0; i<(sizeof(arr)/sizeof(arr[0])); i++)
    {
      ...
      FILE *pasP;
        ...
        if ((pasP = fopen(fp, "r")) == NULL)
	        {
	        	cout<<" could not read password file !"<<endl;
			}
            else
            {
                //cout<<"password directry found"<<endl;
	            pass1=false;
            }
        fclose(pasP);
